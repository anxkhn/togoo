...
function ScoreBar({ score, className }: { score: number; className?: string }) {
  return (
    <div className={cn("flex items-center gap-2", className)}>
      ...
      <span className="text-xs font-medium text-muted w-9 text-right">{Math.round(score * 100)}%</span>
    </div>
  );
}
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-sm text-muted mt-0.5">{formatDuration(durationMinutes)}</p>
        </div>
      ...
      {meeting.explanation && (
        <p className="mt-3 text-xs text-muted border-t border-border pt-3">{meeting.explanation}</p>
      )}
